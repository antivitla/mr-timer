...
angular.module("TimerwoodApp.controllers")
	.controller("MonthViewCtrl", ["$scope", "$rootScope", "$filter", "Months", "Storage", "$timeout", function($scope, $rootScope, $filter, Months, Storage, $timeout) {

		// записи хронологии дней
		$scope.months = Months.months;

		// разделение на недавнее и всё остальное (меняем в опциях)
		$scope.recentCount = 2;
		$rootScope.$watch("monthRecent", function(newval, oldval) {
			$scope.recentCount = newval;
		});
		$scope.recent = function(month) {
			return $scope.months.indexOf(month) > $scope.recentCount-1 ? false : true;
		};
		$scope.excludeRecent = function(month) {
			return $scope.months.indexOf(month) > $scope.recentCount-1 ? true : false;
		};
		$scope.getRecentMonthsButtonText = function() {
			var text = "";
			var date = $scope.months[$scope.recentCount];
			if(!date) date = new Date();
			else date = date.getDate();

			// var day = $filter("filterDateTo")(date, "d");
			var month = $filter("filterDateTo")(date, "MMMM");
			if($scope.recentDates) {
				text = "Скрыть ранние даты";
			} else {
				text = month + " и более ранние";
			}
			return text;
		};
