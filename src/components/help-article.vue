<template lang="pug">
  article(chibi="3")
    ...
    p Чтобы просто записать время один раз, неважно чего - просто шлепните по кнопке. Название сгенерится от балды. Время будет написано на кнопке.

    h3 # Больше одной записи
    p Когда станет две и больше записей, станет доступна "история". Если вы записывали одну и ту же задачу (одинаковое имя), появятся "задачи", где эти две записи сгруппируются в одну "задачу" с суммарной длительностью.

    h3 # Дни и месяцы
    p Появятся на следующий день когда вы запишете ещё что-то. Месяцы - в следующем месяце :)
