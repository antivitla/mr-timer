<template lang="pug">
  article
    ...
    p Когда станет две и больше записей, станут доступны "задачи". Задача - это и есть цель отслеживания - она группирует записей с одинаковыми именами и считает суммарную длительность по ним.

    h3 # Дни и месяцы
    p Так же как и задачи, группируют записи по дням и месяцам. "Дни" появятся на следующий день когда вы запишете ещё что-то (или отредактируете одну из записей чтоб она была другим днём). "Месяцы" - в следующем месяце :)
    ...
    h3 # Вложенные задачи
    p Как только плоский список перестанет удовлетворять, можно делать вложенные задачи. Для этого записывайте названия задач через слеш &laquo;/&raquo;. Это как путь к файлу (через вложенные папки), образующий потенциально сложное дерево задач. Как только появится хотя бы одна вложенная задача, в "задачах" вы увидите как образуется дерево
    ...
    ul
      ...
      h3 # Остальное
      p Есть ещё пакетное удаление в "истории", в сайдбаре можно аву себе сменить, экспортировать и импортировать данные в JSON-формат, переключить язык и валюту (можно и в ссылке через `?ru&amp;rub` например). Ещё можно фильтровать историю, тоже скраешку слева под таймером появится полюшко-поле вводе.
</template>
